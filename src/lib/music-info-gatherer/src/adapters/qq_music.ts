...
export default class QQMusicAdapter extends AbstractAdapter {
    ...
    public async search(options: SearchOptions) {
        const response = await this.fetch({
            url: `/soso/fcgi-bin/client_search_cp?ct=24&qqmusic_ver=1298&new_json=1&remoteplace=txt.yqq.song&searchid=52723111612424300&t=0&aggr=1&cr=1&catZhida=1&lossless=0&flag_qc=0&p=1&n=20&w=${encodeURIComponent(options.songName + ' ' + options.artistName)}&g_tk=688287957&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0`,
        });
